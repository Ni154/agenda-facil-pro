...
export default function Payables() {
  ...
  const [editingPayable, setEditingPayable] = useState<AccountPayable | undefined>();
  ...
  const stats = {
    totalPending: payables.filter(p => p.status === 'pending').reduce((acc, p) => acc + Number(p.amount), 0),
    totalPaid: payables.filter(p => p.status === 'paid').reduce((acc, p) => acc + Number(p.amount), 0),
    totalOverdue: payables.filter(p => p.status === 'pending' && new Date(p.due_date) < new Date()).reduce((acc, p) => acc + Number(p.amount), 0)
  };
    ...
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-stone-500">Gerencie suas despesas e obrigações financeiras.</p>
        </div>
        <button 
          onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-200">
          <p className="text-sm font-medium text-stone-500 mb-1">Total a Pagar</p>
          <h3 className="text-2xl font-bold text-stone-900">{formatCurrency(stats.totalPending)}</h3>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-200">
          <p className="text-sm font-medium text-stone-500 mb-1">Total Pago (Mês)</p>
            ...
            <tbody className="divide-y divide-stone-100">
              {loading ? (
                ...
                filteredPayables.map(payable => (
                  <tr key={payable.id} className="hover:bg-stone-50/50 transition-colors group">
                    <td className="px-6 py-4">
                      <p className="text-sm font-bold text-stone-800">{payable.description}</p>
                      <p className="text-[10px] text-stone-400 font-mono uppercase">Origem: {payable.source_type}</p>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5 text-sm text-stone-600">
                        <Calendar size={14} className="text-stone-400" />
                        <span className={cn(
                          payable.status === 'pending' && new Date(payable.due_date) < new Date() ? "text-red-600 font-bold" : ""
                        )}>
                          {formatDate(payable.due_date as any)}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        payable.status === 'paid' ? "bg-emerald-100 text-emerald-700" : 
                        payable.status === 'pending' ? "bg-amber-100 text-amber-700" : "bg-stone-100 text-stone-500"
                      )}>
                        {payable.status === 'paid' ? <CheckCircle2 size={12} /> : 
                         payable.status === 'pending' ? <Clock size={12} /> : <XCircle size={12} />}
                        {payable.status === 'paid' ? 'Pago' : 
                         payable.status === 'pending' ? 'Pendente' : 'Cancelado'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
