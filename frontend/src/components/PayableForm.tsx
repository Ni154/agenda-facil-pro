...
export default function PayableForm({ payable, onSuccess, onCancel }: PayableFormProps) {
  const { companyUser } = useAuth();
  const companyId = useEffectiveCompany();
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<PayableFormData>({
    ...
    defaultValues: payable ? {
      ...
      payment_method: (payable.payment_method as any) ?? undefined,
    } : {
      status: 'pending',
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-stone-100">
        ...
        >
          {isSubmitting ? 'Salvando...' : payable ? 'Atualizar' : 'Cadastrar'}
        </button>
      </div>
    </form>
  );
